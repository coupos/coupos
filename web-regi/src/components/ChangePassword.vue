<template>
  <div id="change-password">
    <h1>パスワードを変更</h1>
    <p v-if="$route.query.force">セキュリティのためにパスワードを変更する必要があります。</p>
    <form @submit.stop.prevent="submit()" method="post">
      ...
      <a class="button sign-up" @click="submit()" href="#">変更</a>
    </form>
  </div>
</template>
    ...
    validate (only = null) {
      ...
      if (only === null || only === 'newPassword') {
        if (!this.input.newPassword || this.input.newPassword === '') {
          ...
          return false
        } else if (this.input.newPassword === this.input.oldPassword) {
          this.$set(this.error, 'newPassword', '現在のパスワードと同じです')
          return false
        } else {
        ...
        AwsUtil.changePassword(this.input.oldPassword, this.input.newPassword)
          .then(response => {
            ...
            if (this.$route.query.force) {
              AwsUtil.put('CouposAPI', 'regi', { forceChangePassword: false })
                .then(response => {
                  console.log('ユーザー情報を更新 : ' + response.status)
