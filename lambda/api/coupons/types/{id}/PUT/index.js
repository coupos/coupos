...
exports.handler = (event, context, callback) => {
  ...
  let values = [ event.pathParameters.id, event.requestContext.authorizer.claims.sub, request.name, request.value, request.unit, request.target ? JSON.stringify(request.target) : null, request.limit_times, request.concurrent, request.default_expire_time ]

  conn.query('REPLACE INTO `coupon_types` (`id`, `userid`, `name`, `value`, `unit`, `target`, `limit_times`, `concurrent`, `default_expire_time`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', values, (error, result, fields) => {
    if (error) {
      console.log(error)
      conn.destroy()
      callback(error)
